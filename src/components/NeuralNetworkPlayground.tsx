    ...
    for (let layer = 1; layer < 4; layer++) {
      ...
      layerNeurons.forEach(neuron => {
        ...
        prevLayerNeurons.forEach(prevNeuron => {
          ...
          if (connection && prevNeuron && typeof prevNeuron.activation === 'number' && typeof connection.weight === 'number') {
            const weight = connection.weight;
            const activation = prevNeuron.activation;
    ...
    >
      ...
      <div className="relative">
        <canvas
          ...
          width={typeof window !== 'undefined' ? Math.min(800, (window?.innerWidth ?? 800) - 48) : 800}
          height={typeof window !== 'undefined' ? Math.min(400, (window?.innerHeight ?? 400) * 0.5) : 400}
        />
      </div>
    </motion.div>
  );
